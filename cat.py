...
class Cat:
	...
	def move(self, function, best_pos):
		...
		#------SEEKING------
			...
			for j in range(cso_simulation.SMP):
				candidate_moves.append(
					[
						random.uniform(
							self._position[idx_dim] - (self._position[idx_dim] * cso_simulation.SRD) / 100, 
							self._position[idx_dim] + (self._position[idx_dim] * cso_simulation.SRD) / 100
						)
			...
			next_position_idx = np.random.choice(cso_simulation.SMP, 1, p=probabilities)[0]
			...
			for idx_dim in range(self._dimension_size):
				#Compute velocity
				self._velocities[idx_dim] = self._velocities[idx_dim] + r1 * cso_simulation.c1 * (best_pos[idx_dim] - self._position[idx_dim])
				#Apply bounds			
